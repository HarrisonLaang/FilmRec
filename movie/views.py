...
from .models import User, Movie, Genre, Movie_rating, Movie_similarity, Movie_hot, Announcement
...
class IndexView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        # 新增：从数据库里取出最新几条公告
        context['announcements'] = Announcement.objects.filter(is_active=True)[:3]
...
class TagView(ListView):
    ...
    def get_queryset(self):
        ...
        sort = params.get('sort', 'rating')  # 默认按评分

        # 先按类型筛选
        if genre:
            qs = Movie.objects.filter(genre__name=genre)
        else:
            qs = Movie.objects.all()

        # 为评分排序准备平均分
        qs = qs.annotate(avg_score=Avg('movie_rating__score'))

        # 为“年份排序”准备：从 name 里截出最后 4 位作为 year_str
        qs = qs.annotate(
            name_len=Length('name'),
            year_str=Substr('name', Length('name') - 4 + 1, 4)  # 取倒数4位，如 1995
        )
        ...
        if sort == 'title':
            ...
            qs = qs.order_by('year_str')  # 按字符串的 4 位年份排，效果等同数字年
        else:  # rating 或默认
            qs = qs.order_by('-avg_score')
    ...
    def get_context_data(self, **kwargs):
        ...
        params = self.request.GET
        context['genre'] = params.get('genre')  # 让模板能高亮左侧类别
        context['sort'] = params.get('sort', 'rating')
        # 这里保留你原来的分页辅助数据
        paginator = context.get('paginator')
        page_obj = context.get('page_obj')
        pagination_data = self.get_pagination_data(paginator, page_obj)
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            # ===== 1. 先尝试用 Django 内置用户体系（admin 用户）登录 =====
            admin_user = authenticate(request, username=name, password=pwd)
            if admin_user is not None and admin_user.is_staff:
                # 说明这是一个有后台权限的 Django 用户，比如 admin_backstage
                auth_login(request, admin_user)
                return redirect('/admin/')   # 直接进入后台首页

            # ===== 2. 如果不是 admin 用户，再走原来的前台 User 登录逻辑 =====
            user = User.objects.filter(name=name, password=pwd).first()
            if user:
                # 登录成功，在 session 里记录前台用户 id
...
def UserLogout(request):
    # 1. 退出 Django 自带认证（如果当前是 admin_backstage 之类）
    auth_logout(request)

    # 2. 清空当前 session（包括你之前存的 user_id 等）
    request.session.flush()

    # 3. 回到前台首页 /movie/ 对应的 index
    return redirect(reverse('movie:index'))
...
class SplashView(View):
    def get(self, request):
        return render(request, 'movie/splash.html')